...
drop = tf.keras.layers.Dropout(0.5)(outputs)
output = Dense(1)(drop)
# define new model
...
#o =  Flatten()(model.layers[-5].output)
#output = Dense(1)(o)
# define new model
#model = Model(inputs=model.inputs, outputs=output)
      
#setup_to_finetune(model,-3)      
#print(model.summary())
...
# Function that yields random samples of 10k us face images
...
callbacks = [
    tf.keras.callbacks.ModelCheckpoint(filepath="res_hf/drop/epoch_{epoch}.h5"),
    tf.keras.callbacks.CSVLogger("res_hf/drop/faces_ft1.log", separator=",", append=False),
...
model.fit_generator(train_gen, steps_per_epoch=int(len(train_split) / batch_size), epochs=50, verbose=1, 
                    validation_data=valid_gen, validation_steps=int(len(test_split) / batch_size), callbacks = callbacks)
